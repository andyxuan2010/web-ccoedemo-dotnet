...
app.Use(async (context, next) =>
{
    var isHealthPath = context.Request.Path.Equals("/health", StringComparison.OrdinalIgnoreCase) ||
        context.Request.Path.Equals("/healthz", StringComparison.OrdinalIgnoreCase);
    if (isHealthPath && (HttpMethods.IsGet(context.Request.Method) || HttpMethods.IsHead(context.Request.Method)))
    {

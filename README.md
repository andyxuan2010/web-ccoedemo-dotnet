...
- GitHub Actions deployment checks branch-specific Azure auth before deploy and uses repository variables `STAGE_REPOSITORY` (`owner/repository`) and `ADO_REPO_URL` plus secrets `STAGE_REPO_TOKEN` and `ADO_REPO_PAT` for optional mirror publishing.

...
public sealed class HomeController : Controller
{
    ...
    public async Task<IActionResult> Authorized()
    {
        ...
        if (!_authDemoService.ValidateAndConsumeMsalState(HttpContext.Session, state))
        {
            PopulateLayoutViewData();
            return View("~/Views/Error/AuthError.cshtml", new AuthErrorViewModel
            {
                Error = "invalid_state",
                ErrorDescription = "The sign-in response could not be validated. Start a new sign-in attempt."
            });
        }

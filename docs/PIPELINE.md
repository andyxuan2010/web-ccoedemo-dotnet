...
- CI runs on pushes to `main`, `dev`, and `sbx`
- PR validation runs for pull requests targeting `main`, `dev`, and `sbx`
...
The `CreateGitTag` stage runs after `Build` for non-PR runs.

It:

1. Fetches all tags
2. Detects the latest semantic tag
3. Calculates a major, minor, or patch bump from commit messages
4. Creates an annotated tag only when that tag does not already exist locally or remotely

The implementation avoids the old `pipefail` plus `head` pattern that previously produced exit code `141`.
...
- the previous third deployment target path has been removed from the active deploy template
...
- runs on `main`, `dev`, and `sbx`
- uses separate deployment precheck, build, tag, deploy, and mirror-publish jobs
- uses branch-specific Azure credentials for `prod`, `dev`, and `sbx`

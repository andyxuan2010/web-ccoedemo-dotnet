...
5. The job downloads the artifact, resolves up to three App Service targets, checks SCM/Kudu connectivity, and performs ZIP deploy with `az webapp deploy`.
6. An `always()` cleanup step removes the run-specific SCM allow rules after success or failure. A force-cancelled runner can prevent cleanup, so stale `github-actions-*` rules should be monitored.
7. The `pre-publish-check` job validates GitHub Actions repository variables `STAGE_REPO_URL` and `ADO_REPO_URL`, plus secrets `STAGE_REPO_TOKEN` and `ADO_REPO_PAT`.
8. If a mirror-publish variable or secret is missing, the workflow emits a warning and skips only that publish job, matching the sibling repo behavior.
9. When configured, `publish-stage-repo` and `publish-ado-repo` push clean snapshot mirrors to GitHub and Azure DevOps target repos.

...
| --- | --- | --- | --- | --- | --- |
| Deployment trigger | Auto on `main`, with PR validation targeting `main` | Manual by default in this repo | Auto on `main`, plus optional manual deploy toggle on workflow dispatch | Portal sync or connected source trigger | Pipeline run or container image update |
...
2. The `deployment-precheck` job validates the selected GitHub Environment's Azure deployment inputs before the deploy job is allowed to run.
...
4. The deploy job logs into Azure with the service principal credentials, discovers the GitHub runner's public IPv4 address, and temporarily allows that `/32` address on each target's SCM/Kudu site.
5. Before ZIP deployment, the job checks each target for an App Service Deployment Center source binding. When one exists, it logs the bound repository and branch, deletes the binding, and verifies removal with bounded retries.
6. The job downloads the artifact, resolves up to three App Service targets, checks SCM/Kudu connectivity, and performs ZIP deploy with `az webapp deploy`.
7. An `always()` cleanup step removes the run-specific SCM allow rules after success or failure. A force-cancelled runner can prevent cleanup, so stale `github-actions-*` rules should be monitored.
8. The `pre-publish-check` job validates GitHub Actions repository variables `STAGE_REPOSITORY` (`owner/repository`) and `ADO_REPO_URL`, plus secrets `STAGE_REPO_TOKEN` and `ADO_REPO_PAT`.
9. If a mirror-publish variable or secret is missing, the workflow emits a warning and skips only that publish job, matching the sibling repo behavior.
10. When configured, `publish-stage-repo` and `publish-ado-repo` push clean snapshot mirrors and the canonical release tag to GitHub and Azure DevOps target repos.

The source-binding detachment is a compatibility guard for direct ZIP deployment. The Azure service principal must be able to manage `Microsoft.Web/sites/sourcecontrols/*`. If Terraform still declares `deployment_method = "deployment_center"`, a later Terraform apply can recreate the binding; use a ZIP-based deployment method in Terraform to make GitHub Actions the durable deployment owner.

### Required GitHub Actions configuration

- Deploy authentication:
  - Repository variable: `DEPLOY_ENV`
  - Environment secrets: `ARM_CLIENT_ID`, `ARM_CLIENT_SECRET`, `ARM_TENANT_ID`, `ARM_SUBSCRIPTION_ID`
  - Environment variable: `WEBAPP_NAME_PRIMARY`
  - Optional environment variables: `WEBAPP_NAME_SECONDARY`, `WEBAPP_NAME_THIRD`
- Mirror publish:
  - Repository variables: `STAGE_REPOSITORY` (`owner/repository`), `ADO_REPO_URL`
...
- Temporarily changing SCM access restrictions and removing source-control bindings requires broader App Service configuration permissions.
- A force-cancelled workflow can leave a stale SCM allow rule, and Terraform configured for Deployment Center can recreate a detached source binding.

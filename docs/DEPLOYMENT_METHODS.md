...
| --- | --- | --- | --- | --- | --- |
| Deployment trigger | Auto on `main`, PR validation on `main` | Manual by default in this repo | Auto on `main`, `dev`, and `sbx`, plus optional manual deploy toggle on workflow dispatch | Portal sync or connected source trigger | Pipeline run or container image update |
...
9. The pipeline deploys the ZIP with `az webapp deploy --type zip`.
...
- Supports one required target plus two optional extra targets in a single deploy stage.

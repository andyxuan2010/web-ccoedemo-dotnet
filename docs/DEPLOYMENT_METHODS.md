...
8. The `pre-publish-check` job validates GitHub Actions repository variables `STAGE_REPO_URL` and `ADO_REPO_URL`, plus secrets `STAGE_REPO_TOKEN` and `ADO_REPO_PAT`.

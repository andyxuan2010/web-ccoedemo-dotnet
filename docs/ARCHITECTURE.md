...
- The pipeline checks up to three App Service targets before deployment:
  - primary: `webAppNamePrimary`
  - secondary: `webAppNameSecondary`
  - third: `webAppNameThird`
- Secondary and third targets are optional and are skipped when blank or not found.
...
- Deployment then uses `az webapp deploy --type zip`.
...
- No automated unit/integration test project is present.
